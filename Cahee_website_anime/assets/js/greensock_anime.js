let c = $('.gameScene')[0],
    ...
    bg = [],
    fg = {img:new Image(), x:cw, top:40},
    // change place running Dino
    dino = {img:new Image(), spriteOffsetX:0, x:cw/4.5, top:402},
    ...
    // change place fruits(coin)
    coin = {img:new Image(), spriteOffsetX:0, top:265, right:150, show:true};
  
window.onmousemove = function(e){
  gsap.to(dino, {duration:2, x:e.clientX-250, overwrite:'auto'});
};

// window.addEventListener('responsive', function (event) {
...
gsap.fromTo(dust2, {x:function(){return dino.x+70}}, {delay:0.2, duration:0.6, repeat:-1, ease:'sine', x:'-=50', repeatRefresh:true});
...
// Change fruits!
coin.img.src='assets/images/strawberry.png';
// coin2.img.src='assets/images/strawberry_2.png';
// let cTL = gsap.timeline({paused:true}).to(coin, {spriteOffsetX:400, ease:'steps(5)', repeat:4});
let cTL = gsap.timeline({paused:true}).to(coin,{spriteOffsetX:400, ease:'steps(1)', repeat:0});
